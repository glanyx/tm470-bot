...
const data = new SlashCommandBuilder()
  .setName('kick')
  .setDescription('Kick a user by specified User ID')
  .setDMPermission(false)
  .setDefaultMemberPermissions(PermissionFlagsBits.KickMembers)
  .addStringOption(option =>
    option
      .setName('user_id')
      .setDescription('The User ID to kick.')
      .setRequired(true)
  )
  .addStringOption(option =>
    option
...
export class Kick extends SlashCommand {
  ...
  public static async run({
    ...
    guild.members.kick(member, reason)
      .then(() => {
        interaction.editReply(`${member} was kicked.\nReason: ${reason}`)
        ...
        GuildSetting.fetchByGuildId(guild.id)
          .then(async setting => {
            ...
            if (ch && ch.isTextBased()) ch.send(`${member} was kicked.\nReason: ${reason}`)
          })
          .catch(e => {
      ...
      .catch(e => {
        interaction.editReply(`Error, ${member} was not kicked from the server.\nReason:\n${e.message}`)
      })
